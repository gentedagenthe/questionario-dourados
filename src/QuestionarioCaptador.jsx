...
const estilos = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .intro-card { background: #fff; border-radius: 12px; padding: 0; border: 1px solid #D4E6F1; overflow: hidden; box-shadow: 0 2px 10px rgba(27,111,171,0.08); }
  .intro-topo { background: linear-gradient(135deg, #1B6FAB, #15517f); padding: 28px 26px 22px; color: #fff; }
  .intro-cargo { font-size: 1.35rem; font-weight: 800; margin-bottom: 6px; }
  .intro-tags { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 10px; }
  .intro-tag { background: rgba(255,255,255,0.18); padding: 4px 12px; border-radius: 20px; font-size: 0.78rem; font-weight: 600; }
  .intro-corpo { padding: 24px 26px; }
  .intro-secao { margin-bottom: 20px; }
  .intro-secao h3 { font-size: 0.92rem; font-weight: 700; color: #1B6FAB; margin-bottom: 10px; display: flex; align-items: center; gap: 6px; }
  .intro-lista { list-style: none; display: flex; flex-direction: column; gap: 7px; }
  .intro-lista li { font-size: 0.88rem; color: #444; padding-left: 18px; position: relative; line-height: 1.45; }
  .intro-lista li::before { content: "•"; color: #6BBF4E; font-weight: 800; position: absolute; left: 0; }
  .banner-prazo { background: #15517f; color: #fff; padding: 12px 16px; text-align: center; font-size: 0.85rem; font-weight: 600; }
  .lgpd-intro { background: #f0f4f8; border: 1.5px solid #D4E6F1; border-radius: 10px; padding: 16px; margin: 0 26px 24px; }
  .lgpd-intro p { font-size: 0.82rem; color: #555; line-height: 1.6; margin-bottom: 10px; }
  .lgpd-intro label { display: flex; gap: 10px; align-items: flex-start; font-size: 0.85rem; font-weight: 600; color: #222; cursor: pointer; }
  .lgpd-intro input[type="checkbox"] { width: 17px; height: 17px; margin-top: 1px; flex-shrink: 0; accent-color: #1B6FAB; }
  .btn-iniciar-wrap { padding: 0 26px 28px; }
  .btn-iniciar { width: 100%; background: #1B6FAB; color: #fff; border: none; font-weight: 700; font-size: 1rem; padding: 13px; border-radius: 8px; cursor: pointer; font-family: inherit; }
  .btn-iniciar:hover:not(:disabled) { opacity: 0.9; }
  .btn-iniciar:disabled { opacity: 0.45; cursor: not-allowed; }
...
export default function QuestionarioCaptador() {
  ...
  const [lgpdIntro, setLgpdIntro] = useState(false);
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    if (!lgpdIntro) { setErro("Você precisa aceitar os termos de uso para enviar o questionário."); return; }
    setEnviando(true);
    setErro("");
    const payload = {
      ...
      <div style={{ background: "#f0f4f8", minHeight: "100vh" }}>
        ...
        <div className="container">
          ...
          <div className="intro-card">
            <div className="intro-topo">
              <div className="intro-cargo">📣 Captador Comercial</div>
              <div className="intro-tags">
                <span className="intro-tag">📍 Dourados/MS</span>
                <span className="intro-tag">🏢 Crédito e Serviços Financeiros</span>
                <span className="intro-tag">📄 CLT</span>
              </div>
            </div>

            <div className="banner-prazo">⏰ Vagas limitadas — preencha o quanto antes</div>

            <div className="intro-corpo">
              <div className="intro-secao">
                <h3>💰 Remuneração</h3>
                <ul className="intro-lista">
                  <li>Salário: R$ 3.500,00</li>
                  <li>Comissão por captação de clientes</li>
                </ul>
              </div>
              ...
              <div className="intro-secao">
                ...
                <ul className="intro-lista">
                  <li>07:30 às 18:00 horas, com intervalo de almoço</li>
                </ul>
              </div>
              ...
              <div className="intro-secao">
                <h3>📚 Requisitos</h3>
                <ul className="intro-lista">
                  <li>Ensino Superior completo ou cursando em Administração, Gestão Comercial, Marketing, Contabilidade ou áreas correlatas</li>
                  <li>Experiência na área comercial, vendas externas ou prospecção de clientes</li>
                  <li>Boa comunicação, negociação e relacionamento interpessoal</li>
                  <li>Conhecimento em Pacote Office e ferramentas de CRM será um diferencial</li>
                  <li>Possuir CNH e disponibilidade para visitas externas</li>
                </ul>
              </div>
              ...
              <div className="intro-secao">
                ...
                <ul className="intro-lista">
                  <li>Prospectar novos clientes e parceiros comerciais para operações de crédito</li>
                  <li>Realizar visitas externas a empresas, comerciantes, produtores rurais e potenciais tomadores de crédito</li>
                  <li>Desenvolver e fortalecer relacionamentos comerciais de longo prazo</li>
                  <li>Identificar oportunidades de negócios e apresentar soluções financeiras adequadas</li>
                  <li>Alimentar e acompanhar o funil de vendas no sistema interno</li>
                </ul>
              </div>
              ...
              <div className="intro-secao">
                ...
                <ul className="intro-lista">
                  ...
                  <li>Forte orientação para resultados</li>
                  <li>Excelente capacidade de comunicação e negociação</li>
                  <li>Organização, disciplina e comprometimento</li>
                </ul>
              </div>
            </div>

            <div className="lgpd-intro">
              <p>
                Antes de iniciar, é importante saber que suas respostas serão utilizadas exclusivamente
                pela Genthe Consultoria para fins de avaliação deste processo seletivo, em conformidade
                com a Lei Geral de Proteção de Dados (Lei 13.709/2018). Seus dados não serão compartilhados
                com terceiros sem autorização.
              </p>
              <label>
                <input type="checkbox" checked={lgpdIntro} onChange={e => setLgpdIntro(e.target.checked)} />
                <span>Li e concordo com os termos de uso e tratamento dos meus dados pessoais (LGPD).</span>
              </label>
            </div>

            <div className="btn-iniciar-wrap">
              <button className="btn-iniciar" disabled={!lgpdIntro} onClick={() => setIniciado(true)}>
